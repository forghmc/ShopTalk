...
def clear_all():
    state.value = []  # Reset the conversation history stored in state
    return [], state  # Return the reset state and empty the chatbot
"""
Gradio Blocks low-level API that allows to create custom web applications (here our chat app)
"""
with gr.Blocks() as demo:
    logger.info("Starting Demo...")
    chatbot = gr.Chatbot(label="ShopTalk")
    msg = gr.Textbox()
    state = gr.State([])
    clear = gr.Button("Clear", elem_id="clear_button")
    clear.click(clear_all, [], [chatbot, state])
